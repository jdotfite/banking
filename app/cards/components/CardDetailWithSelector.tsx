...
import CardSelector from '@/components/ui/card/CardSelector';
import LoadingSpinner from '@/components/ui/common/LoadingSpinner';
import { ArrowLeft, Bell } from 'lucide-react';
import Link from 'next/link';
// CSS is now in globals.css

interface CardDetailWithSelectorProps {
  cardId: string;
}

const CardDetailWithSelector: React.FC<CardDetailWithSelectorProps> = ({ cardId }) => {
  ...
  const handleCardSelect = (newCardId: string) => {
    ...
    router.push(`/cards/${newCardId}`, { scroll: false });
  };
  ...
  const selectedCard = userData?.creditCards?.find(card => card.id === selectedCardId);
        ...
        <div className="px-5 mx-auto max-w-md mt-10 text-center">
          <div className="bg-[#212121] rounded-xl p-6">
            <p className="text-red-500 mb-4">
              The card with ID "{selectedCardId}" could not be found.
            </p>
      ...
      <div className="px-5 pt-8 pb-6 mx-auto max-w-md">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div className="text-xl font-medium tracking-tight">SELECT A CREDIT CARD</div>
          </div>
      ...
      <div className="px-5 mx-auto max-w-md">
        ...
        <div className="bg-blue-600 rounded-xl p-6 mb-6">
          ...
          <div className="space-y-4">
            <button className="w-full py-4 bg-white bg-opacity-10 rounded-xl text-left px-6 text-xl font-semibold hover:bg-opacity-20 transition-all">
              Payment
            </button>
            <button className="w-full py-4 bg-white bg-opacity-10 rounded-xl text-left px-6 text-xl font-semibold hover:bg-opacity-20 transition-all">
              Activity
        ...
        <div className="bg-[#212121] rounded-xl p-6 mb-6">
          <div className="flex justify-between mb-4">
            <div>
              ...
              <p className="text-neutral-400">•••• {selectedCard.cardNumber.slice(-4)}</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-medium">${selectedCard.currentBalance.toFixed(2)}</p>
          ...
          <div className="flex justify-between">
            <div>
              ...
              <p className="text-green-500 font-medium">${selectedCard.availableCredit.toFixed(2)}</p>
            </div>
            <div className="text-right">
              <p className="text-neutral-400">Due Date</p>
              <p className="text-white font-medium">{new Date(selectedCard.dueDate).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <Link href={`/cards/manage/${selectedCardId}`} className="bg-[#212121] rounded-xl p-4 flex flex-col items-center justify-center hover:bg-neutral-800 transition-colors">
...
export default CardDetailWithSelector;
