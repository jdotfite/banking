Set-Content -Path './components/ui/transactions/TransactionList.tsx' -Value '// components/ui/transactions/TransactionList.tsx
...
const TransactionList: React.FC<TransactionListProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (data && data.groupedTransactions && data.groupedTransactions.user1) {
      setTransactionGroups(data.groupedTransactions.user1);
    } else {
...
export default TransactionList;' -Force
